...
{ 
	...
	m_meshes.reserve(64);
	loadShader("Color", "./shaders/simpleShader.vert", "./shaders/colorShader.frag");
...
void loadTexture(const char*name,const char*path, int rows = 1, int cols = 1 )
{
	TextureData t = LoadTexture(path, rows, cols);
...
void drawLine(vec3, vec3 b, vec4 color);
